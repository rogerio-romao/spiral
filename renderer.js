...
function chooseAlgos() {
    ...
    switch (choose) {
        ...
        case 'nuclear-vortex':
            ...
            break;
        case 'tricordion':
            displayAlgos('TRICORDION');
            ctx.save();
            runningAlgo = new Tricordion();
            runningAlgo.draw();
            break;
        case 'acid-stars':
        ...
        case 'warp2001':
            ...
            break;
        case 'golden-spiral':
            displayAlgos('GOLDEN SPIRAL');
            ctx.save();
            runningAlgo = new GoldenSpiral();
            runningAlgo.draw();
            break;
        case 'carnival-ride':
            displayAlgos('CARNIVAL RIDE');
            ctx.save();
            runningAlgo = new CarnivalRide();
            runningAlgo.draw();
            break;
        case 'porthole':
        ...
        case 'supernova':
            ...
            runningAlgo.draw();
            break;
        case 'crosses-naughts':
            displayAlgos('CROSSES NAUGHTS');
            ctx.save();
            runningAlgo = new CrossesNaughts();
            runningAlgo.draw();
            break;
        case 'organic':
        ...
        case 'shadowy':
            ...
            break;
        case 'mondrianacci':
            displayAlgos('MONDRIANACCI');
            ctx.save();
            runningAlgo = new Mondrianacci();
            runningAlgo.draw();
            break;
        case 'radio-waves':
        ...
        case 'perspective':
            ...
            break;
        case 'the-jester':
            displayAlgos('THE JESTER');
            ctx.save();
            runningAlgo = new TheJester();
            runningAlgo.draw();
            break;
        case 'angel-hair':
            ...
            break;
        case 'dreamcatcher':
            displayAlgos('DREAMCATCHER');
            ctx.save();
            runningAlgo = new Dreamcatcher();
            runningAlgo.draw();
            break;
        case 'sushi':
            displayAlgos('SUSHI');
    ...
    constructor() {
        ...
        this.draw = () => {
            ...
            if (t % speed === 0) {
                ...
                if (stagger === 0) {
                    ctx.lineTo(
                        round(w / 2 + this.length),
                        round(h / 2 - this.length)
                    );
                }
                ...
                if (stagger === 3) {
                    ctx.lineTo(
                        round(w / 2 + this.length - this.gap),
                        round(h / 2 + this.length)
                    );
...
class Pentagons {
    constructor() {
        ...
        ctx.strokeStyle = randomColor(5, 255, 0.3, 0.3);
...
class Hexagons {
    constructor() {
        ...
        ctx.shadowBlur = 10;
        ctx.shadowColor = ctx.strokeStyle = randomColor(5, 255, 0.3, 0.3);
...
class Spiderwebs {
    constructor() {
        ...
        ctx.shadowColor = ctx.strokeStyle = randomColor(5, 255, 0.9, 0.9);
        ctx.shadowBlur = 5;
...
class Paintings {
    constructor() {
        this.length = random(50, Math.min(w, h) / 1.5);
        ctx.strokeStyle = randomColor(5, 255, 0.8, 0.8);
        ctx.fillStyle = randomColor(5, 255, 0.05, 0.05);
        ...
        this.draw = () => {
            ...
            if (t % (speed * 20) === 0) {
                ...
                ctx.fillStyle = ctx.fillStyle = randomColor(5, 255, 0.8, 0.8);
                ctx.stroke();
                ctx.fill();
                ctx.fillStyle = ctx.fillStyle = randomColor(5, 255, 0.05, 0.05);
            }
            if (t % (speed * 77) === 0) {
...
class Stargate {
    constructor() {
        ...
        ctx.fillStyle = randomColor(0, 255, 0.8, 0.8);
...
class FlawedMirror {
    constructor() {
        this.letters = [
            33, 34, 42, 43, 65, 90, 94, 166, 175, 177, 191, 192, 195, 198, 199,
            204, 208, 210, 212, 247, 256, 294, 298
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        );
...
class ThePlatter {
    constructor() {
        this.letters = [
            302, 306, 308, 310, 313, 323, 324, 325, 327, 328, 332, 336, 337,
            345, 350, 354, 358, 363, 365, 366, 370, 371, 373, 375, 380, 381,
            383, 388, 390, 399
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        ...
        ctx.textAlign = 'center';
        ctx.strokeStyle = randomColor(0, 255, 0.06, 0.06);
        this.draw = () => {
            if (t % speed === 0) {
                ctx.strokeText(this.letter, w / 2, h / 2);
                ctx.translate(w / 2, h / 2);
...
class LightBringer {
    constructor() {
        this.letters = [
            501, 502, 503, 508, 509, 522, 525, 526, 528, 530, 533, 534, 538,
            547, 558, 562, 567, 589
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        ...
        this.draw = () => {
            ...
            if (t % (speed * 360) === 0) {
                ...
                ctx.globalCompositeOperation = this.light
                    ? 'lighter'
                    : 'multiply';
                this.letter = String.fromCharCode(
                    this.letters[random(0, this.letters.length)]
...
class CakeDecorator {
    constructor() {
        this.letters = [
            801, 803, 805, 808, 809, 810, 811, 812, 816, 817, 820, 822, 823,
            826, 827, 829, 831, 832, 836, 837, 839, 840, 841, 843, 845, 846,
            848, 850, 857, 858, 859, 860, 866, 894
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        );
        ...
        this.draw = () => {
            ...
            if (t % (speed * 60) === 0) {
                ...
                ctx.fillStyle = randomColor(10, 255, 0.05, 0.05);
                this.fontSize = random(50, 350);
                ctx.font = 'bold ' + this.fontSize + 'px serif';
            }
            if (t % (speed * 360) === 0) {
...
class TribalTatoo {
    constructor() {
        this.letters = [
            901, 903, 926, 931, 932, 933, 935, 938, 939, 943, 944, 947, 953,
            954, 955, 964, 965, 967, 970, 978, 979, 983, 986, 987, 989, 990,
            993, 995, 999
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        ...
        ctx.fillStyle = randomColor(10, 255, 0.5, 0.5);
        ctx.lineWidth = 3;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.font = 'bold ' + this.fontSize + 'px serif';
...
class Tricordion {
    constructor() {
        this.side = random(15, 100);
        this.change = random(1, 200);
        this.rotate = random(3, 357);

        ctx.globalAlpha = 0.2;
        ctx.shadowColor = 'white';
        ctx.shadowBlur = 60;
        ctx.filter = 'saturation(0.45)';
        ctx.lineWidth = 2;
...
class GoldenSpiral {
    constructor() {
        this.letters = [
            1701, 1703, 1704, 1705, 1706, 1707, 1712, 1713, 1714, 1718, 1722,
            1723, 1724, 1726, 1727, 1728, 1729, 1731, 1734, 1736, 1737, 1744,
            1746, 1748, 1749, 1750, 1752, 1755, 1758, 1759, 1760, 1761, 1763,
            1768, 1769, 1770, 1771, 1773, 1774, 1776, 1777, 1779, 1781, 1784,
            1788, 1789, 1790, 1792, 1793, 1794, 1795
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        );
        ...
        this.rotations = [
            1, 2, 3, 4, 5, 7, 8, 10, 11, 12, 14, 15, 16, 17, 19, 20, 21, 23, 24,
            26, 27, 28, 29, 30, 32, 33, 34, 35, 36, 38, 39, 41, 42, 43, 45, 46,
            47, 48, 50, 51, 52, 54, 55, 56, 57, 58, 59, 60, 64, 65, 66, 67, 69,
            70, 73, 75, 76, 77, 78, 81, 82, 83, 84, 85, 88, 89
        ];
        this.rotate = this.rotations[random(0, this.rotations.length)];

        ctx.strokeStyle = randomColor(0, 255, 0.6, 1);
        ...
        this.draw = () => {
            if (t % speed === 0) {
                ...
                ctx.rotate(this.rotations[random(0, this.rotations.length)]);
                this.x *= 1.618;
                this.y *= 1.618;
                if (this.x > Math.max(w, h)) this.x = 1;
                if (this.y > Math.max(w, h)) this.y = 1;
                ctx.translate(-w / 2, -h / 2);
            }
            t++;
            if (t % (speed * 150) === 0) {
...
class CarnivalRide {
    constructor() {
        this.letters = [
            3103, 3104, 3105, 3107, 3108, 3109, 3111, 3112, 3114, 3115, 3118,
            3119, 3122, 3128, 3129, 3133, 3136, 3157, 3162, 3174, 3175, 3176,
            3179, 3183, 3192, 3193, 3194, 3199
        ];
        this.letter = String.fromCharCode(
            this.letters[random(0, this.letters.length)]
        );
        this.fontSize = random(40, 280);
        this.rot1 = random(1, 5);
        this.rot2 = random(2, 6);
        this.rot3 = random(3, 7);
        this.rot4 = random(4, 8);
        this.rot5 = random(-5, -1);

        ctx.fillStyle = randomColor(0, 255, 0.4, 1);
        ctx.strokeStyle = randomColor(10, 175, 0.5, 0.75);
        ctx.shadowColor = 'black';
        ctx.shadowOffsetX = 4;
        ctx.shadowOffsetY = 4;
        ctx.shadowBlur = 5;
        ctx.lineWidth = random(15, 40);
        ctx.textAlign = 'center';
        ctx.font = this.fontSize + 'px serif';
        ...
        this.draw = () => {
            if (t % speed === 0) {
                ...
                if (stagger === 1) {
                    ...
                    ctx.moveTo(w * 0.125, h / 2);
                    ctx.lineTo(w * 0.875, h * 0.125);
                    ctx.stroke();
                    ctx.translate(-w * 0.75, -h * 0.25);
                    ctx.restore();
                }
                if (stagger === 2) {
                    ...
                    ctx.moveTo(w / 2, h * 0.125);
                    ctx.lineTo(w * 0.875, h * 0.875);
                    ctx.stroke();
                    ctx.translate(-w * 0.75, -h * 0.75);
                    ctx.restore();
                }
...
class CrossesNaughts {
    constructor() {
        this.radius = random(20, 300);

        ctx.strokeStyle = randomColor(0, 255, 0.1, 0.5);
        ctx.lineWidth = random(5, 30);
        ...
        this.draw = () => {
            if (t % speed === 0) {
                ...
                if (this.radius % 2 === 0) {
                    ...
                    this.radius = this.radius * 3 + 1;
                }
            }
            if (this.radius === 1 || this.radius > Math.max(w, h) * 8) {
                this.radius = random(20, 300);
                ctx.beginPath();
                ctx.strokeStyle = randomColor(0, 255, 0.1, 0.5);
                ctx.lineWidth = random(5, 30);
...
class Mondrianacci {
    constructor() {
        this.first = 0;
        this.second = 1;
        this.divisors = [2, 3, 4, 6, 8, 10, 12, 16];
        this.divisor = this.divisors[random(0, this.divisors.length)];
        this.seq = [this.first, this.second];

        ctx.lineWidth = random(1, 6);
        ctx.strokeStyle = randomColor(0, 255, 0.45, 1);

        this.draw = () => {
            if (t % speed === 0) {
                let side = this.seq[0] + this.seq[1];
                this.seq.push(side);
                this.seq.shift();
                if (side > Math.max(w, h)) {
                    this.first++;
                    this.second++;
                    this.seq = [this.first, this.second];
                    ctx.translate(w / 2, h / 2);
...
class Dreamcatcher {
    constructor() {
        ...
        this.x3 = random(0, w);
        this.y3 = random(0, h);
        this.rotate = (random(1, 360) * Math.PI) / 180;
        ...
        this.draw = () => {
            if (t % speed === 0) {
                stagger = stagger % 4;
                if (stagger === 0) {
                    ctx.moveTo(this.x2++, this.y3);
                    ctx.lineTo(this.y1, this.x3);
                    ctx.stroke();
                }
                if (stagger === 1) {
                    ctx.moveTo(this.x1++, this.y1--);
...
class TheJester {
    constructor() {
        ...
        this.x2 = random(0, w);
        this.y2 = random(0, h);
        this.radius1 = random(3, 30);
        this.radius2 = random(3, 30);
        this.rotate = (random(1, 360) * Math.PI) / 180;
    ...
    constructor() {
        ...
        this.draw = () => {
            if (t % speed === 0) {
                ...
                this.cornerX =
                    this.cornerX + 1 >= this.cornersX.length
                        ? 0
                        : this.cornerX + 1;
                this.cornerY =
                    this.cornerY + 1 >= this.cornersY.length
                        ? 0
                        : this.cornerY + 1;
                ctx.fill();
                ctx.beginPath();
